...
import pandas as pd
from difflib import get_close_matches  # Corrected import for clarity

# Load the data (assuming 'movies.csv' is in the same directory)
try:
...
# Load the saved model (assuming 'movie_model.sav' is in the same directory)
try:
...
if selected == 'Movie Recommendation System':
    ...
    # Find close matches using get_close_matches for clarity
    list_of_titles = movie_data['title'].tolist()
    close_matches = get_close_matches(movie_name, list_of_titles, n=1, cutoff=0.8)  # Consider using a cutoff for better accuracy
